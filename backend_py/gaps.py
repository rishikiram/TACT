...
class Gap_001(Gap):
    ...
    def severity_score(self) -> tuple:
        ...
        if not (self.get_claim_status(self.SOC_comparator_claim) == "supported"):
            score += 2
            new_rationale += "Standard of Care comparator is present; "

        if not (self.get_claim_status(self.placebo_comparator_claim) == "supported"):
            score += 1
            new_rationale += "Placebo comparator is present; "

        if not (self.get_claim_status(self.direct_comparator_claim) == "supported"):
            score += 1
            new_rationale += "Direct comparator is present; "

        if not (self.get_claim_status(self.indirect_comparator_claim) == "supported"):
            score += 1
...
class Gap_002(Gap):
    ...
    def severity_score(self) -> tuple:
        score = 1 # GAP_SEVERITY_ENUM[4] == supported
        new_rationale = []
        if not (self.get_claim_status(self.os_claim) == "supported"):
            score += 3
            new_rationale += ["Overall Survival is shown"]

        if not (self.get_claim_status(self.pfs_claim) == "supported"):
            score += 2
            new_rationale += ["Significant progression free survival is shown"]

        if not (self.get_claim_status(self.orr_claim) == "supported"):
            score += 1
